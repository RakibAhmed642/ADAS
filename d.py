...
# FORCE GPU CHECK (LOCAL-FRIENDLY)
...
if torch.cuda.is_available():
    ...
    if torch.version.cuda is None:
        print("⚠️ GPU detected on your PC ≠ PyTorch can use it.")
        print("Reason: Your installed PyTorch is CPU-only (no CUDA build).")
        print("Fix (NVIDIA GPU required):")
        print("  python -m pip uninstall -y torch torchvision torchaudio")
        print("  python -m pip install --index-url https://download.pytorch.org/whl/cu121 torch torchvision torchaudio")
        print("If you don't have NVIDIA (CUDA), CPU is expected.")
    else:
        print("⚠️ PyTorch has CUDA support, but CUDA is not available at runtime.")
        print("Common causes: missing/old NVIDIA driver, unsupported GPU, or driver mismatch.")
        print("Try updating your NVIDIA driver, then re-run.")
    print("Running on CPU (this may be slower).")
    print("=" * 50 + "\n")
    device = "cpu"

print("Loading MAX POWER Model (Extra Large) for Ultimate Accuracy...")
# Using 'yolov8x.pt' (Extra Large) for best detection
...
# 5. EXECUTION (LOCAL WEBCAM/VIDEO)
# -----------------------------------
if __name__ == "__main__":
    print("Initializing ADAS V34.0 (ENHANCED UI & MINIMAP)...")
    ...
    def find_available_cameras(max_index=5):
        """
        Scan for cameras and return a list of (index, backend) that actually return frames.
        """
        available = []
        ...
        for backend in backends:
            for idx in range(max_index + 1):
                ...
                cap_try.release()
                if ok:
                    available.append((idx, backend))
        return available

    # Optional: pass a video file path as first argument: python "car dtec 2.py" video.mp4
    if len(sys.argv) > 1:
        source = sys.argv[1]
        ...
        cams = find_available_cameras(max_index=5)
        if not cams:
            print("❌ Could not find any working camera.")
            print("Try closing other apps using the webcam (Zoom/Teams/Browser), then run again.")
            sys.exit(1)

        print("\nAvailable cameras:")
        for i, (idx, backend) in enumerate(cams):
            print(f"  [{i}] index={idx}, backend={_backend_name(backend)}")

        # User chooses which one to use
        choice_str = input(f"Select camera number [0-{len(cams)-1}] (default 0): ").strip()
        if choice_str == "":
            choice = 0
        else:
            try:
        ...
        cam_idx, cam_backend = cams[choice]
        print(f"✅ Using camera index={cam_idx}, backend={_backend_name(cam_backend)}")
        cap = cv2.VideoCapture(cam_idx, cam_backend)
        if not cap.isOpened():
            print("❌ Failed to open selected camera. Exiting.")
            sys.exit(1)

    # Set HD Resolution (optional)
    ...
    while True:
        ...
        cv2.imshow("ADAS V34", final_frame)
