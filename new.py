...
try:
    from otvld_net import OTVLDNet
except ImportError:
    OTVLDNet = None
...
class ADASProcessor:
    def __init__(self):
        ...
        # Lane & Traffic Flow History
        self.lane_history = deque(maxlen=20)
        self.traffic_flow_speeds = deque(maxlen=60) # Store recent vehicle speeds
        ...
        # OTVLD-Net Integration (Advanced Lane Detection)
        self.otvld_model = None
        self.otvld_device = 'cuda' if torch.cuda.is_available() else 'cpu'
        if OTVLDNet is not None:
            try:
                self.otvld_model = OTVLDNet().to(self.otvld_device)
                weights_path = 'e:/DTC/otvld_weights.pt'
                if os.path.exists(weights_path):
                    self.otvld_model.load_state_dict(torch.load(weights_path, map_location=self.otvld_device))
                    print("✅ OTVLD-Net Lane Detection Model Loaded Correctly.")
                else:
                    print("⚠️ OTVLD-Net weights not found. Forcing ADAS to run untrained model to verify processing pipeline performance (FPS) as requested.")
                self.otvld_model.eval()
            except Exception as e:
                print(f"⚠️ Error loading OTVLD-Net: {e}. Falling back to traditional Lane Detection.")
                self.otvld_model = None
    ...
    def analyze_environment(self, frame, vehicle_count):
        ...
        # Adaptive deep weather filtration filter parameters
        self.needs_dehaze = self.weather_status in ["FOGGY", "RAIN"]
    ...
    def apply_advanced_dehaze_filter(self, frame):
        # Extremely fast pseudo-dehaze via local histogram equalization and unsharp masking
        blur = cv2.GaussianBlur(frame, (0, 0), 3)
        unsharp = cv2.addWeighted(frame, 1.5, blur, -0.5, 0)
    ...
    def apply_night_vision(self, frame, boxes):
        ...
        # Optional: Weather adaptive dehaze override if conditions are poor during daytime
        # (Already applied night CLAHE above if it was dark, so we don't double filter easily)
        if hasattr(self, 'needs_dehaze') and self.needs_dehaze and brightness >= 70:
            frame = self.apply_advanced_dehaze_filter(frame)
            mode_text = "DEHAZE ACTIVE"
    ...
    def draw_augmented_grid(self, frame, min_ttc_val=100.0):
        h, w = frame.shape[:2]
        horizon = int(0.50 * h)
        
        y_10m = int(horizon + self.focal_length * (self.cam_height / 10.0))
        
        # --- Advanced Dynamic 3D Perspective Safety Carpet ---
        pt1_left = (int(0.23 * w), h)
        pt2_left = (int(0.4475 * w), y_10m)
        pt1_right = (int(0.77 * w), h)
        pt2_right = (int(0.5525 * w), y_10m)
        
        # Dynamic Color Logic Based on Global Minimum TTC of Ego Lane
        carpet_color = (0, 255, 0) # Base Green
        if min_ttc_val < 5.0: carpet_color = (0, 165, 255) # Yellow Warning Space
        if min_ttc_val < 2.5: carpet_color = (0, 0, 255) # Red Critical Brake Zone
        
        overlay_poly = frame.copy()
        poly_pts = np.array([pt1_left, pt2_left, pt2_right, pt1_right], np.int32)
        cv2.fillPoly(overlay_poly, [poly_pts], carpet_color)
        
        # Opacity gradient - pulses when extremely dangerous
        carpet_alpha = 0.08
        if min_ttc_val < 2.5:
             carpet_alpha = 0.15 + (math.sin(time.time() * 15) * 0.05) # Pulse
             
        frame = cv2.addWeighted(overlay_poly, carpet_alpha, frame, 1.0 - carpet_alpha, 0)
        
        # 2. Crisp boundary lines matching carpet color
        overlay_lines = frame.copy()
        cv2.line(overlay_lines, pt1_left, pt2_left, carpet_color, 2)
        cv2.line(overlay_lines, pt1_right, pt2_right, carpet_color, 2)
    ...
    def detect_lane_lines(self, frame):
        # 1. Advanced OTVLD-Net Deep Learning Interference (If Active)
        if self.otvld_model is not None:
            try:
                h, w = frame.shape[:2]
                
                # Dynamic pre-processing for Transformer & ODConv
                img_resized = cv2.resize(frame, (224, 224))
                img_rgb = cv2.cvtColor(img_resized, cv2.COLOR_BGR2RGB)
                
                # Normalize exactly like ImageNet bounds for best performance
                input_tensor = torch.from_numpy(img_rgb).float() / 255.0
                mean = torch.tensor([0.485, 0.456, 0.406]).to(self.otvld_device)
                std = torch.tensor([0.229, 0.224, 0.225]).to(self.otvld_device)
                
                input_tensor = input_tensor.to(self.otvld_device)
                input_tensor = (input_tensor - mean) / std
                input_tensor = input_tensor.permute(2, 0, 1).unsqueeze(0) # (1, 3, 224, 224)
                
                with torch.no_grad():
                    out = self.otvld_model(input_tensor)
                
                # Post-processing OTVLD network predictions (Heatmap + Offsets -> Physical lines)
                lane_heatmap = out['heatmap'][0].cpu() # (C, H, W)
                lane_scores = out['object_score'][0].cpu() # (C,)
                
                valid_lines = []
                c = lane_heatmap.shape[0]
                
                thresh = 0.5
                # Decode physical lines based on Omni-dimensional prediction heatmaps
                for idx in range(c):
                    if lane_scores[idx] > thresh:
                        prob_map = lane_heatmap[idx]
                        
                        # Simplified decoding matching heatmap peak points across y-axis
                        max_y_indices = torch.argmax(prob_map, dim=1) # Width dimension
                        
                        # Extract two extreme valid horizontal points to form a solid segment
                        pts = []
                        hm_h, hm_w = prob_map.shape
                        for yi in range(hm_h):
                            xi = max_y_indices[yi].item()
                            if prob_map[yi, xi] > thresh:
                                mapped_x = int((xi / hm_w) * w)
                                mapped_y = int((yi / hm_h) * h)
                                pts.append((mapped_x, mapped_y))
                        ...
                        if len(pts) > 1:
                            ...
                            # Retain near-vertical constraint
                            if abs(slope) > 0.4:
                ...
                # Silently catch and gracefully drop into fallback to prevent crash while driving
                pass
                
        # 2. Traditional Canny+Hough Fallback (Guarantees stability if model is absent/fails)
    ...
    def detect_vehicle_signals(self, frame, box, track_id):
        ...
        x1, y1, x2, y2 = map(int, box)
        w, h = x2 - x1, y2 - y1
        if w < 40 or h < 30: return "STRAIGHT"

        # ROI: Focus on upper/middle/lower rear (tail lights can be anywhere from 20% to 80% height)
        # We capture a bit higher to ensure we get the US (Upper Signal / Center High Mount Stop Light)
        lights_y1 = y1 + int(h * 0.15)
        lights_y2 = y1 + int(h * 0.85)
        if lights_y1 >= lights_y2: return "STRAIGHT"

        roi = frame[lights_y1:lights_y2, x1:x2]
        if roi.size == 0: return "STRAIGHT"

        # --- Meta-Learning Geometric Post-Processing (PPL) Regions ---
        roi_h, roi_w = roi.shape[:2]
        
        # 1. LIS (Left Indicator Signal) - Far left 25% width
        w_lis = int(roi_w * 0.25)
        lis_roi = roi[:, :w_lis]
        
        # 2. RIS (Right Indicator Signal) - Far right 25% width
        ris_roi = roi[:, roi_w - w_lis:]
        
        # 3. US (Upper Signal / Center Stop Lamp) - Top 40% height, Center 30% width
        # This is strictly used for deterministic braking verification
        us_w_start = int(roi_w * 0.35)
        us_w_end = int(roi_w * 0.65)
        us_h_end = int(roi_h * 0.40)
        us_roi = roi[:us_h_end, us_w_start:us_w_end]

        # Analysis Function - Adaptive thresholding over PPL zones
        def get_ppl_light_score(img_part):
            if img_part.size == 0: return 0, 0
            hsv = cv2.cvtColor(img_part, cv2.COLOR_BGR2HSV)
            lab = cv2.cvtColor(img_part, cv2.COLOR_BGR2LAB)
            l_channel = lab[:, :, 0]
            
            # Adaptive Brightness: Find the peak brightness in this specific zone
            # This makes the detection robust against glare and dynamic lighting
            _, max_val, _, _ = cv2.minMaxLoc(l_channel)
            
            # If the whole zone is dark, return 0
            if max_val < 100: return 0, max_val
            
            # Threshold relative to the peak brightness of the zone (top 20% brightest pixels)
            adaptive_thresh = max(100, max_val * 0.8)
            bright_mask = (l_channel > adaptive_thresh)
            ...
            score = red_pixels + (amber_pixels * 3.0)
            
            # Return normalized score by area
            area = max(1, img_part.shape[0] * img_part.shape[1])
            return score / area, max_val

        l_score, _ = get_ppl_light_score(lis_roi)
        r_score, _ = get_ppl_light_score(ris_roi)
        us_score, us_peak = get_ppl_light_score(us_roi)
        
        if track_id not in self.intensity_history:
            self.intensity_history[track_id] = {'L': deque(maxlen=24), 'R': deque(maxlen=24), 'US': deque(maxlen=24)}
            
        self.intensity_history[track_id]['L'].append(l_score)
        self.intensity_history[track_id]['R'].append(r_score)
        self.intensity_history[track_id]['US'].append(us_score)
        
        hist_L = list(self.intensity_history[track_id]['L'])
        hist_R = list(self.intensity_history[track_id]['R'])
        hist_US = list(self.intensity_history[track_id]['US'])
        ...
        mean_US = np.mean(hist_US)
        
        ptp_L = np.max(hist_L) - np.min(hist_L) # Peak-to-Peak (Amplitude of the flash)
        ptp_R = np.max(hist_R) - np.min(hist_R)
        ...
        # 1. Deterministic Brake Detection (BIS) via Upper Signal (US)
        # The Center High Mount Stop Light is the most reliable indicator of braking,
        # perfectly reducing ambiguity from LIS/RIS glow.
        us_active = mean_US > active_thresh * 0.5 
        
        # 2. Advanced Blinker Detection - High Peak-to-Peak amplitude in LIS/RIS
        left_blinking = ptp_L > blink_amplitude and np.max(hist_L) > active_thresh
        right_blinking = ptp_R > blink_amplitude and np.max(hist_R) > active_thresh
        
        # 3. Decision Logic resolving ambiguity
        if us_active and not (left_blinking or right_blinking):
            return "BRAKE"
            
        # Standard Brake if US is missing/occluded but both sides are brightly lit and steady
        ...
        if left_blinking and not right_blinking:
            if np.max(hist_L) > np.max(hist_R) * 1.2:
                return "SIGNAL LEFT"
        elif right_blinking and not left_blinking:
            if np.max(hist_R) > np.max(hist_L) * 1.2:
                return "SIGNAL RIGHT"
    ...
    def draw_hud(self, frame, mode_text, lane_type, safety_score, live_stats, active_signs):
        ...
        # COLLECTIVE TRAFFIC FLOW AGGREGATOR DISPLAY
        if len(self.traffic_flow_speeds) > 5:
            avg_flow_speed = int(sum(self.traffic_flow_speeds) / len(self.traffic_flow_speeds))
            flow_status = "FREE FLOW"
            flow_color = (0, 255, 0)
            
            if avg_flow_speed < 20 and live_stats.get('car', 0) > 2:
                flow_status = "CONGESTION"
                flow_color = (0, 0, 255)
            elif avg_flow_speed < 45 and live_stats.get('car', 0) > 1:
                flow_status = "SLOW FLOW"
                flow_color = (0, 165, 255)
                
            y_flow = state_y + 120
            
            overlay_f = frame.copy()
            cv2.rectangle(overlay_f, (10, y_flow), (150, y_flow + 50), (20, 20, 20), -1)
            frame = cv2.addWeighted(overlay_f, 0.5, frame, 0.5, 0)
            cv2.rectangle(frame, (10, y_flow), (150, y_flow + 50), flow_color, 1)
            
            self.draw_text_with_outline(frame, f"FLOW: {avg_flow_speed} KM/H", (15, y_flow + 20), 0.4, (255, 255, 255))
            self.draw_text_with_outline(frame, flow_status, (15, y_flow + 40), 0.4, flow_color)
    ...
    def process_frame(self, frame):
        ...
        # We process the frame through night vision / dehaze first
        frame, mode_text = self.apply_night_vision(frame, vehicle_boxes_only)
        
        # NOTE: draw_augmented_grid requires min_ttc_val for the 3D safety carpet, so we defer it to AFTER object processing!
                ...
                if is_on_road and label in self.colors:
                    ...
                    if self.track_lifetime.get(track_id, 0) > 5 or is_cut_in:
                        ...
                        if label != 'traffic light' and label != 'stop sign':
                            if track_id != -1 and not is_braking and velocity > 1.0: # Filter outliers
                                self.traffic_flow_speeds.append(speed_kmh)
                        ...
                        if label != 'traffic light':
                            ...
                            # --- VRU GROUND BUBBLE LOGIC ---
                            if label in ['person', 'bicycle', 'motorcycle', 'dog', 'cow'] and stable_dist < 40:
                                b_cx, b_cy = (x1 + x2) // 2, y2
                                
                                # Perspective accurate bubble width based on distance
                                base_radius = int(max(width, y2-y1) * 0.6)
                                ...
                                overlay_bubble = frame.copy()
                                if crossing_intent and not in_ego_lane:
                                    # Yellow pre-warning bubble
                                    cv2.ellipse(overlay_bubble, (b_cx, b_cy), (base_radius, int(base_radius*0.3)), 0, 0, 360, (0, 165, 255), -1)
                                    frame = cv2.addWeighted(overlay_bubble, 0.4, frame, 0.6, 0)
                                    cv2.putText(frame, "CROSSING INTENT", (b_cx-40, y1-20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 165, 255), 2)
                                elif in_ego_lane or stable_dist < 10:
                                    # Red Critical Warning bubble
                                    bubble_alpha = 0.4 + (math.sin(time.time() * 20) * 0.2)
                                    cv2.ellipse(overlay_bubble, (b_cx, b_cy), (base_radius, int(base_radius*0.3)), 0, 0, 360, (0, 0, 255), -1)
                                    frame = cv2.addWeighted(overlay_bubble, bubble_alpha, frame, 1.0 - bubble_alpha, 0)
                                    cv2.ellipse(frame, (b_cx, b_cy), (base_radius, int(base_radius*0.3)), 0, 0, 360, (0, 0, 255), 2)
                                    cv2.putText(frame, "WATCH OUT!", (b_cx-40, y1-20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                                else:
                                    # Safe state ground bubble projection
                                    cv2.ellipse(overlay_bubble, (b_cx, b_cy), (base_radius, int(base_radius*0.3)), 0, 0, 360, (0, 255, 0), -1)
                                    frame = cv2.addWeighted(overlay_bubble, 0.2, frame, 0.8, 0)
                                    cv2.ellipse(frame, (b_cx, b_cy), (base_radius, int(base_radius*0.3)), 0, 0, 360, (0, 255, 0), 1)
        ...
        # AFTER ALL OBJECTS PROCESSED: Draw the 3D dynamic safety carpet now that we know the true global minimum TTC of ego lane objects
        frame = self.draw_augmented_grid(frame, min_ttc_val)

        # ADVANCED HUD RETICLE on most critical object
        if critical_obj_center:
            rcx, rcy = critical_obj_center
            # Spinning and pulsing fighter-jet reticle mathematics
            rot_angle = time.time() * 3.0
            pulse_radius = 20 + int(math.sin(time.time() * 15) * 5)
            
            for offset in [0, math.pi/2, math.pi, 3*math.pi/2]:
                px1 = int(rcx + math.cos(rot_angle + offset) * (pulse_radius - 8))
                py1 = int(rcy + math.sin(rot_angle + offset) * (pulse_radius - 8))
                px2 = int(rcx + math.cos(rot_angle + offset) * pulse_radius)
                py2 = int(rcy + math.sin(rot_angle + offset) * pulse_radius)
                cv2.line(frame, (px1, py1), (px2, py2), (0, 0, 255), 2)
                
            cv2.circle(frame, critical_obj_center, 3, (0, 255, 255), -1)
